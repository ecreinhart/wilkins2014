...
wilkins2014_raw <- read.csv("../data/wilkins & kaiser 2014 replication_November 28, 2018_21.54.csv") 

#remove first two rows that are headers and not data
wilkins2014_filtered <- wilkins2014_raw[-(1:2),]

#select variables of interest 
wilkins2014_selected <- wilkins2014_filtered %>% 
  select(slb_1:slb_12,awb_1:awb_8,condition,rac_prog,race,attn_check_1,attn_check_2)

#### Data exclusion / filtering  

#exclusions
wilkins2014_excluded <- wilkins2014_selected %>% 
  filter(race == 5, #filter White participants only 
         attn_check_1 == 7, #select if followed attn check instructing select "7, strongly agree"
         !is.na(attn_check_2)) #select if followed attn check instructing "do not select a response"

#add arbitrary participant ID
wilkins2014_id <- wilkins2014_excluded %>% 
  rowid_to_column("ID")

#tidy and change to numeric variables 
wilkins2014_gathered = wilkins2014_id %>% 
  gather("measurement", "value", 
         -ID, -condition, -rac_prog, -race, -attn_check_1, -attn_check_2) %>% 
...
manip_check <- t.test(wilkins2014_id$rac_prog ~ wilkins2014_d$condition)
print(manip_check)
