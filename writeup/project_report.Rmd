...
wilkins2014 <- read.csv("wilkins & kaiser 2014 replication_October 29, 2018_00.10.csv")
...
```{r}
#I currently do not yet know how to run a hierarhcial linear regression in R, so I did a few other significance tests that provide some information about the hypothesis (e.g., is the effect of condition on anti-White bias different for a high-SLB sample vs low-SLB sample). I will continue researching how to do a hierarchial linear regression and plan to have the code ready next week. 

#split mean SLB beliefs into above and below the mean 
wilkins2014 <- wilkins2014 %>% 
  mutate(slb_mean = mean(slb_composite)) %>% 
  mutate(slb_split = ifelse(slb_composite > slb_mean,"H","L"))
print(wilkins2014$slb_mean)
View(wilkins2014)

#select high_SLB, run t.test for condition differences on awb
wilkins2014_slb_high <- wilkins2014 %>% 
  filter(slb_split == "H")
View(wilkins2014_slb_high)
slb_high_t <- t.test(wilkins2014_slb_high$awb_composite ~ wilkins2014_slb_high$condition)
print(slb_high_t)

#select low_SLB, run t.test for condition differences on awb 
wilkins2014_slb_low <- wilkins2014 %>% 
  filter(slb_split == "L")
View(wilkins2014_slb_low)
slb_low_t <- t.test(wilkins2014_slb_low$awb_composite ~ wilkins2014_slb_low$condition)
print(slb_low_t)

#hierarhcial linear regression will go here 

#reproduce the plot in the paper will go here 
